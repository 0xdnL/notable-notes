...
## read two certificates at the same time
...
openssl crl2pkcs7 \
  -nocrl \
  -certfile <(cat .kube/config | yq '.users[] | select(.name == "onpremise-prod" ) | .user.client-certificate-data | @base64d') \
  -out multi_cert.p7b

openssl pkcs7 -print_certs -in multi_cert.p7b -text|grep After
```
